# BS-ComputerMonitor — Agent Context
...
- **Theme:** Custom per-metric `Theme` struct with a `ThemeId` enum (6 themes: CoralStorm, Shibui, Kasane, ColdSteel, Jizo, HotSteel — matching `Rust-Desktop-Standards.md`). Default is `ColdSteel`, per the standard. `theme_id` is persisted in `CardVisibility`/`config.json`. Live switching via `prev_theme_id` diff in `update()`. Metric accents (cpu/mem/gpu/net/disk/temp) — and the hover/active glow, which is fixed to `ACCENT_CPU` — are shared constants across all themes; only surfaces and text colours change per theme.
